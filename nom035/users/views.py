...
from . import choices
from .models import Employee, InformationLog
...
def index_view(request):
    if request.user.is_authenticated:
        ...
        if company is not None:
            ...
            sheets = employee.answer_sheets.filter(final_answer="unanswered").order_by(
                "-id"
            )
            for sheet in sheets:
...
def register_employee(request):
    if request.method == "POST":
        ...
        InformationLog.objects.create(
            employee=employee,
            civil_status=civ_st,
            educational_level=educ_lvl,
            position=pos,
            area=area,
            position_type=pos_type,
            contract_type=contr_type,
            personnel_type=pers_type,
            working_day_type=work_d_type,
            shift_rotation=shft_rot,
            current_position_time=curr_pos_t,
            work_experience_time=work_exp_t,
        )
    return redirect("user:employees_view")
...
def company_index_data(surveys):
    ...
    if surveys.count() == 1 or surveys[0].guide_number == surveys[1].guide_number:
        ...
        graph_data.append(
            surveys[1].answer_sheets.filter(final_answer="Muy alto").count()
        )
        ...
        try:
            ...
            for employee in employees:
                ...
                if sheets:
                    if (
                        sheets.count() == 1
                        or sheets[0].survey.guide_number
                        == sheets[1].survey.guide_number
                    ):
...
def info_log_string(info_log):
    info = {
        ...
        "current_position_time": choices.current_position_time[
            str(info_log.current_position_time)
        ],
        "work_experience_time": choices.work_experience_time[
            str(info_log.work_experience_time)
        ],
    }
    return info
        ...
        if pwd != "":
            if pwd != pwd_conf:
                ...
                if valid_pwd == 0:
                    ...
                    messages.success(
                        request, "Su contraseña fue actualizada con exito!"
                    )
                else:
                    messages.error(request, valid_pwd)
    return redirect("index_view")
